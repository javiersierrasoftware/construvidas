  ...
  if (loading) {
    return (
      <main className="min-h-screen flex items-center justify-center pt-24 pb-16 px-4">
        <p className="text-slate-800">Cargando estado de la orden...</p>
      </main>
    );
  }
    ...
    return (
      <main className="min-h-screen flex items-center justify-center pt-24 pb-16 px-4">
        <div className="text-center p-6 bg-[#111] rounded-lg border border-white/10 max-w-md w-full">
          ...
          <Link href="/tienda" className="bg-secondary-600 text-black font-bold py-2 px-4 rounded-md hover:bg-secondary-500 transition">
            Volver a la Tienda
          </Link>
        </div>
      </main>
    ...
    return (
      <main className="min-h-screen flex items-center justify-center pt-24 pb-16 px-4">
        <div className="text-center p-6 bg-[#111] rounded-lg border border-white/10 max-w-md w-full">
          ...
          <p className="text-slate-700 mb-6">La orden que buscas no existe o ha sido eliminada.</p>
          <Link href="/tienda" className="bg-secondary-600 text-black font-bold py-2 px-4 rounded-md hover:bg-secondary-500 transition">
            Volver a la Tienda
          </Link>
        </div>
      ...
      <div className="text-center p-6 bg-[#111] rounded-lg border border-white/10 max-w-md w-full">
        ...
        <div className="mb-6 space-y-2">
            ...
            {order.items.map((item: any) => (
                <div key={item.productId} className="flex justify-between items-center text-sm text-slate-800">
                    <span>{item.name} x {item.qty}</span>
                    <span>${(item.price * item.qty).toLocaleString()}</span>
                </div>
            ))}
            <div className="flex justify-between items-center text-lg font-bold text-slate-900 border-t border-white/10 pt-2 mt-2">
                <span>Total:</span>
                <span>${order.totalAmount.toLocaleString()}</span>
            </div>
        </div>

        <Link href="/tienda" className="bg-secondary-600 text-black font-bold py-2 px-4 rounded-md hover:bg-secondary-500 transition inline-flex items-center gap-2">
