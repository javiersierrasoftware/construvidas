  ...
  if (loading) {
    return (
      <main className="flex flex-col items-center justify-center min-h-[70vh] p-8">
        ...
        <p className="mt-4 text-xl text-slate-700">Cargando inscripciones...</p>
      </main>
    );
  }
    ...
    <main className="p-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6 mb-8">
        <div className="bg-gray-800/50 p-6 rounded-lg text-center border border-white/10">
          <p className="text-2xl font-bold text-slate-900">{filteredRegistrations.length}</p>
          <p className="text-slate-700 text-sm">Total Inscritos</p>
        </div>
        <div className="bg-gray-800/50 p-6 rounded-lg text-center border border-white/10">
          <p className="text-2xl font-bold text-green-400">{filteredRegistrations.filter(reg => reg.status === "COMPLETED").length}</p>
          <p className="text-slate-700 text-sm">Pagadas</p>
        </div>
        <div className="bg-gray-800/50 p-6 rounded-lg text-center border border-white/10">
          <p className="text-2xl font-bold text-yellow-400">{filteredRegistrations.filter(reg => reg.status === "PENDING_PAYMENT").length}</p>
          <p className="text-slate-700 text-sm">Pendientes</p>
        </div>
        <div className="bg-gray-800/50 p-6 rounded-lg text-center border border-white/10">
          <p className="text-2xl font-bold text-pink-400">{filteredRegistrations.filter(reg => reg.gender === "Mujer").length}</p>
          <p className="text-slate-700 text-sm">Mujeres</p>
        </div>
        <div className="bg-gray-800/50 p-6 rounded-lg text-center border border-white/10">
          <p className="text-2xl font-bold text-blue-400">{filteredRegistrations.filter(reg => reg.gender === "Hombre").length}</p>
          <p className="text-slate-700 text-sm">Hombres</p>
        </div>
        {eventDetails?.slotsLeft !== undefined && (
           <div className="bg-gray-800/50 p-6 rounded-lg text-center border border-white/10">
             <p className="text-2xl font-bold text-accent-500">{eventDetails.slotsLeft - filteredRegistrations.filter(reg => reg.status === "COMPLETED").length}</p>
             <p className="text-slate-700 text-sm">Cupos Restantes</p>
           </div>
        )}
      </div>
      ...
      {filteredRegistrations.length === 0 ? (
        <p className="text-slate-700 text-lg">No hay inscripciones que coincidan con los filtros.</p>
      ) : (
        <div className="overflow-x-auto bg-gray-800/50 rounded-lg shadow-xl">
          <table className="min-w-full divide-y divide-gray-700">
            <thead className="bg-gray-700">
              <tr>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-800 uppercase tracking-wider">
                  Nombre
                </th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-800 uppercase tracking-wider">
                  Email
