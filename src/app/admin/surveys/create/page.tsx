...
function CreateSurveyPageContent() {
  ...
  const handleSave = async (isPublic: boolean = true) => {
    ...
    <main className="min-h-screen pt-24 pb-20 px-6 max-w-4xl mx-auto">
      <div className="mb-10 flex items-center justify-between">
        ...
        <div className="flex gap-3">
          ...
          >
            <Save size={18} /> {loading ? "Guardando..." : "Publicar Encuesta"}
          </button>
        </div>
      </div>
      ...
      <div className="space-y-8">
            ...
            <div key={q.id} className="bg-white border border-slate-200 rounded-[2rem] p-8 shadow-sm group">
              <div className="flex items-center gap-4 mb-6">
                ...
                <select 
                  ...
                  value={q.type}
                  onChange={(e) => updateQuestion(q.id, { type: e.target.value as QuestionType, options: (e.target.value !== 'text' && q.options.length === 0) ? ['Opción 1'] : q.options })}
                >
        ...
        <div className="bg-slate-900 rounded-[2rem] p-4 flex justify-center gap-4 sticky bottom-8 shadow-2xl">
          <button
            ...
            className="flex items-center gap-2 text-white hover:text-secondary-400 transition font-bold uppercase tracking-widest text-[10px] px-4 py-2"
          >
            <Type size={18} /> Texto
          </button>
          <button
            ...
            className="flex items-center gap-2 text-white hover:text-secondary-400 transition font-bold uppercase tracking-widest text-[10px] px-4 py-2"
          >
            <CheckCircle2 size={18} /> Unica
          </button>
          <button
            onClick={() => addQuestion('multiple')}
            className="flex items-center gap-2 text-white hover:text-secondary-400 transition font-bold uppercase tracking-widest text-[10px] px-4 py-2"
          >
            <List size={18} /> Múltiple
          </button>
        </div>
      </div>
    </main>
